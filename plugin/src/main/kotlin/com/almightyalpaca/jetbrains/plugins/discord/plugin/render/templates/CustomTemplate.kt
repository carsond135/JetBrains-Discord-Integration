...
object Utils {
    private fun getVarValue(varName: String, context: CustomTemplateContext): String? =
        when (varName) {
            ...
            "FileIsWritable" -> context.fileData?.fileIsWritable.toString()
    ...
    private fun sizeAsString(size: Int?): String? {
        ...
        if (size < 2 shl 10) return "$size bytes"
        if (size < 2 shl 20) return "${twoDecimals(size.toDouble() / (1 shl 10))} KiB"
        if (size < 1 shl 30) return "${twoDecimals(size.toDouble() / (1 shl 20))} MiB"
        return "${twoDecimals(size.toDouble() / (1 shl 30))} GiB"
    }
