...
class DiagnoseService : DisposableCoroutineScope {
    ...
    enum class Plugins(val message: String) {
        ...
        MULTIPLE("It seems like you have multiple other Rich Presence plugins installed. Please uninstall them to avoid conflicts!")
    }

    enum class Ide(val message: String) {
        SNAP("${ApplicationNamesInfo.getInstance().fullProductName} is running as a Snap package. This will most likely prevent the plugin from connecting to your Discord client!"),
        OTHER("")
    }
}
